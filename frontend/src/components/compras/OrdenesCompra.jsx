...
// Mostrar lista desplegable reutilizable
const ListaDesplegable = ({ items, onSelect, renderItem }) => {
  if (!items || items.length === 0) return null;

  return (
    <ul style={{
      position: 'absolute', zIndex: 1060, top: '100%', left: 0, right: 0,
      background: '#fff', border: '1px solid #dee2e6', borderRadius: 6,
      boxShadow: '0 4px 16px rgba(0,0,0,0.12)', listStyle: 'none',
      margin: 0, padding: '4px 0', maxHeight: 230, overflowY: 'auto'
    }}>
      {items.map((item, i) => (
        <li
          key={i}
...
const BuscadorProveedor = ({ valor, onSeleccionar }) => {
  ...
  const filtrados = todos.filter(p =>
    texto.trim() && p.nombre_proveedor.toLowerCase().includes(texto.toLowerCase())
  );

  // Seleccionar proveedor encontrado
  const seleccionar = (p) => {
...
const BuscadorProducto = ({ onAgregar, idsUsados }) => {
  ...
  // Seleccionar producto y agregarlo al detalle
...
const FormOrden = ({ titulo, ocNumero, lineasIniciales = [], formInicial, onClose, onGuardar }) => {
  ...
  const [proveedor, setProveedor] = useState(
    formInicial.cod_proveedor
      ? {
          cod_proveedor: formInicial.cod_proveedor,
          nombre_proveedor: formInicial.nombre_proveedor || ''
        }
      : null
  );
  ...
  // Actualizar datos de una línea
  const actualizarLinea = (idx, campo, valor) => {
    setLineas(prev => prev.map((l, i) => {
      if (i !== idx) return l;

      const u = { ...l, [campo]: valor };
      u.subtotal = parseFloat((
        (parseFloat(u.precio || 0) + parseFloat(u.isv || 0)) *
        parseInt(u.cantidad || 1)
      ).toFixed(2));
              ...
              <div className="mb-3">
                ...
                <div className="d-flex gap-2 align-items-stretch">
                  <div className="flex-grow-1">
                    <BuscadorProducto
                      key={keyProd}
                      idsUsados={lineas.map(l => l.cod_producto)}
                    ...
                    {lineas.length === 0 ? (
                      ...
                      <tr key={i}>
                        ...
                        <td className="text-end align-middle fw-semibold">
                          {fmtMoneda(l.subtotal, form.moneda)}
                        </td>
...
const OrdenesCompra = () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setSeleccionados(prev =>
          prev.filter(id => data.datos.some(o => o.cod_orden_compra === id))
        );
      }
    } catch {
  ...
  // Seleccionar o deseleccionar orden
  ...
  // Seleccionar o deseleccionar toda la página
  ...
  const aprobarOrden = async (id) => {
    try {
      await comprasService.cambiarEstado(id, {
        cod_estado_oc: 2,
  ...
  const eliminarSeleccionados = async () => {
    const cancelables = ordenes.filter(
      o => seleccionados.includes(o.cod_orden_compra) && o.estado === 'Cancelada'
    );
    ...
    if (eliminadas > 0) {
      toast.success(`${eliminadas} orden(es) eliminada(s)`);
    }
      ...
      {seleccionados.length > 0 && (
        ...
        >
          <span style={{ fontSize: 13, fontWeight: 600, color: '#2563eb' }}>
            ...
            {ordenes.filter(o =>
              seleccionados.includes(o.cod_orden_compra) && o.estado !== 'Cancelada'
            ).length > 0 && (
                  ...
                  <tr key={o.cod_orden_compra}>
                    ...
                    <td className="text-center">
                      <button
                        ...
                        onClick={() => verDetalle(o.cod_orden_compra)}
                        title="Ver"
                      >
                        <FiEye />
                      </button>
                      ...
                      <button
                        ...
                        onClick={() => verEditar(o.cod_orden_compra)}
                        title="Editar"
                      >
                        <FiEdit2 />
                      </button>
                      ...
                      {o.estado === 'Pendiente' && (
                        <button
                          ...
                          onClick={() => aprobarOrden(o.cod_orden_compra)}
                          title="Aprobar"
                        >
                          <FiCheck />
                        </button>
                      ...
                      {!['Recibida', 'Cancelada'].includes(o.estado) && (
                        <button
                          ...
                          onClick={() => cancelarOrden(o.cod_orden_compra)}
                          title="Cancelar"
                        >
                          <FiX />
                        </button>
                      ...
                      {o.estado === 'Cancelada' && (
                        <button
                          ...
                          onClick={() => eliminarOrden(o.cod_orden_compra)}
                          title="Eliminar"
                        >
                          <FiTrash2 />
                        </button>
