...
class OrdenCompraService {
  ...
  async listarEstados() {
    const res = await pool.query(`SELECT cod_estado_oc, nombre, orden FROM cat_estado_orden_compra WHERE activo = 1 ORDER BY orden`);
    return res.rows;
  }
  ...
  async productosDisponibles(buscar = '') {
    const res = buscar?.trim()
      ? await pool.query(`
          ...
          WHERE p.estado_producto = true AND p.nombre_producto ILIKE $1
